...
is clearly feasible, and the coefficients of the LHS are bounded by $\poly(\|r\|, \frac{1}{\delta}, \frac{1}{\epsilon})$. There are $O(n^k)$ variables, so by Cramer's rule, the coefficients of the $\lambda_i'$ can be taken to be bounded by $\poly(\|\cP\|^{n^k}, \frac{1}{\delta}, \frac{1}{\epsilon}, \|r\|, n!)$. Since $\|\cP\|, \|r\| \leq 2^{\poly(n^d)}$ as they are considered part of the input, and $k\geq d$, this bound is $2^{\poly(n^k, \log \frac{1}{\delta}, \log \frac{1}{\epsilon})}$.
